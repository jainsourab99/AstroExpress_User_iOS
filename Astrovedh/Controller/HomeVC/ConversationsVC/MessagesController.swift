...
//        checkIfUserLoggedIn()
    ...
    func observeUserMessages() {
        if let currentUserId = Auth.auth().currentUser?.uid {
            ...
            userMessagesRef.child(currentUserId).observe(.childAdded, with: { (snapshot) in
                ...
                let messageRef = Database.database().reference().child(chatTableName)
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        Database.database().reference().child(userTableName).child(toId).observeSingleEvent(of: .value, with: { (snapshot) in
...
//    }
    ...
    func fetchUserAndSetupNavbarTitle() {
        Database.database().reference().child(userTableName).child((Auth.auth().currentUser?.uid)!).observeSingleEvent(of: .value, with: { (snapshot) in
            if let dictionary = snapshot.value as? [String: AnyObject] {
                let user = User()
                user.name = dictionary["name"] as? String
...
////        }
//
//        let bashboardVC = DashBoardViewController()
//        bashboardVC.messageController = self
//        bashboardVC.modalPresentationStyle = .fullScreen
//        //loginController.messageController?.messages.removeAll()
//        //loginController.messageController?.messagesDictionary.removeAll()
//        self.present(bashboardVC, animated: true, completion: nil)
